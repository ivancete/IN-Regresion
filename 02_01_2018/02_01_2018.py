...
#features.drop(['Alley', 'PoolQC', 'Fence', 'FireplaceQu', 'MiscVal', 'MiscFeature', 'LotFrontage',
...
## Saving to CSV
pd.DataFrame({'Id': test.Id, 'SalePrice': Final_labels}).to_csv('submissionElasticNet.csv', index=False)
...
## Getting our SalePrice estimation
Final_labels = (np.exp(LR_model.predict(test_features)) + np.exp(ENST_model.predict(test_features_st))) / 2

## Saving to CSV
pd.DataFrame({'Id': test.Id, 'SalePrice': Final_labels}).to_csv('submissionLinealRegression.csv', index=False)
'''
'''
Random Forest
'''
